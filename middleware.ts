...
// NOTE: The updateSession function in lib/supabase/middleware.ts should now ONLY return NextResponse
// It should look similar to the standard implementation:
/*
export async function updateSession(request: NextRequest) {
  let response = NextResponse.next({ request: { headers: request.headers } });
  const supabase = createServerClient(..., { cookies: { /* handlers that modify 'response' * / } });
  await supabase.auth.getUser(); // Refreshes session via cookies on 'response'
